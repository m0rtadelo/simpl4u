...
const parent = this.getParentWebComponent();
let p = '';
if (parent) {
  p = ` (parent: ${parent.tagName.toLowerCase()})`;
  //console.log('Soy hijo de', parent.tagName.toLowerCase());
} else {
  p = ' (top-level)';
  //console.log('Estoy directo en el DOM');
}      
      console.log(`Rendering component: ${this.getTagName()} ${this.getName()}${p}...`);
  ...
  */
  ...
  /**
   * Returns the closest ancestor that is a custom element.
   * It checks both the light DOM parent chain and the shadow host chain.
   * @returns {Element|null} The nearest parent web component, or null if none exists.
   */
  getParentWebComponent() {
    ...
    if (rootNode instanceof ShadowRoot && rootNode.host?.tagName?.includes('-')) {
  ...
  /**
   * Returns true when this component is not nested inside another web component.
   * @returns {boolean} True if the element is a top-level DOM component.
   */
  isTopLevelDomComponent() {
    return this.getParentWebComponent() === null;
  }
  ...
  async loadViewState() {
    this.refresh();
    return;
    console.log('load', this.getName());
    await this.loadDebouncedUserData();
    console.log('refresh', this.getName());
    this.refresh();
  }
