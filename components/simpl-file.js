...
export class SimplFile extends FormElement {
  ...
  template(state) {
    ...
    <div class="mb-3" ${this.hidden ? 'style="display:none"' : ''}>
      ${ this.label ? `<label for="${this.getName()}" class="form-label col-12">${LanguageService.i18n(this.label)}${super.isRequired() ? ' <span style="color: var(--bs-form-invalid-color)">*</span>' : ''}</label>` : ''}
      <input (input)="change" id="${this.getName()}" ${super.isRequired()} ${this.disabled ? 'disabled' : ''} class="form-control col-12" type="file" value="${TextService.htmlEscape(state[this.getName()] || '')}"></input>
    </div>
    `;
  }
   ...
   */
  focus() {
    document.querySelector('input#' + this.getName()).focus();
  } 
}
