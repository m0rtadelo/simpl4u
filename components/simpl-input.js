...
export class SimplInput extends FormElement {
  template(state) {
    ...
    <div class="mb-3" ${this.hidden ? 'style="display:none"' : ''}>
      ...
      <input autofocus="true" (input)="change" id="${this.name || this.id}" ${super.isRequired()} ${this.disabled ? 'disabled' : ''} class="form-control col-12" type="${this.type || 'text'}" value="${state[this.name || this.id] || ''}"></input>
    </div>
    `;
  }
